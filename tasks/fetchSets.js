...
function resolveSetUrl(str) {
    ...
    if (parts.length === 2)
        return PROVIDERS.github(parts[0], parts[1], branch);

    throw new Error(`Invalid set string: '${str}'`);
}

export async function fetchSetJson(url) {
    const text = fs.readFileSync('sets/TPA/set.json', 'utf-8');
...
export async function fetchSets(config) {
    ...
    for (const str of config.sets) {
        ...
        data.imageExportPath = [url, data.imageExportPath].join('/');
